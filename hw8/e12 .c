/*
 * e12.c
 * ѕо убыванию и возрастанию
 * —читать массив из 10 элементов и отсортировать первую половину по возрастанию, а вторую - по убыванию.
 * ƒанные на входе: 10 целых элементов массива через пробел
 * ƒанные на выходе: »сходный массив. ѕерва€ часть отсортирована по возрастанию, втора€ половина по убыванию. 
 */
...
void arrSwap (int *arr, int i, int j) // помен€ть элементов массива местами 
{
	int flag = arr[i];
	arr [i] = arr [j];
	arr [j] = flag;
